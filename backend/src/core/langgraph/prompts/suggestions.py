...
**CRITICAL – OPTION COUNT RULE**:
- Generate **exactly ONE renovation option for EACH style listed in popular_styles**.
- The total number of options MUST equal the number of styles provided.
- There is NO maximum or minimum limit when contractor knowledge is provided.
- Do NOT merge styles, skip styles, or invent new styles.

If popular_styles contains 3 styles → generate 3 options  
If popular_styles contains 7 styles → generate 7 options

If **NO contractor knowledge is provided** (popular_styles is missing or empty):
- Generate **4–5 distinct renovation options** based on general renovation trends.

For each style in popular_styles, create ONE complete renovation option using ONLY that style’s characteristics.
